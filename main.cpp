...
void runOnImage(String name) {
	...
	for (int i=0;i<suppressed.size();i++) {
		...
		bool label;
		if ((boost.predict(out) == 1 && checkGroundTruth(p1, p2)) || (boost.predict(out) == 0 && !checkGroundTruth(p1, p2))) label = true;
		precision += (int) label;
		cout << boost.predict(out, Mat(), CV_WHOLE_SEQ, false, true) << " " << label << endl;


		//boost.predict(out) == 1 checkGroundTruth(p1, p2)
	...
	cout << (float) precision / suppressed.size() << endl;
	//imshow("im1", im1);
	//waitKey(0);
}
